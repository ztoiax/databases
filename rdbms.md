...
* [RDBMS (关系性数据库)](#rdbms-关系性数据库)
  ...
  * [分库分表](#分库分表)
    ...
    * [数据迁移问题](#数据迁移问题)
      ...
      * [聚合函数问题：order by、group by等](#聚合函数问题order-bygroup-by等)
      * [分库分表后，分页查询问题](#分库分表后分页查询问题)
...
- [苏三说技术：分库分表 带来了哪些问题？](https://mp.weixin.qq.com/s/4lcUbVjy4VKDhd7T-sKADw)
...
- 问题：扩容（如从 4 个分片扩展到 8 个分片）时，旧数据需要迁移到新分片，迁移复杂且可能影响线上服务。
...
- 分库分表后，假设两个表在不同的数据库（如订单表在库 A，库存表在库 B），无法使用单库事务，导致可能会出现数据的一致性问题。
...
- 解决方法：
    - 1.分布式事务框架`Seata`：支持跨库的分布式事务。
    - 2.柔性事务：使用消息中间件实现最终一致性。
        - 典型实现：RocketMQ 消息事务。
...
#### 聚合函数问题：order by、group by等

- 分库分表后，聚合查询（如总数统计、分页查询）需要跨多个分片表执行，增加了查询复杂度。

    - 例子：查询所有订单总数，需要跨 10 个订单表聚合。按创建时间分页查询所有订单。
...
- 解决方案
    ...
    - 2.手动分片查询：按分片逐一查询数据，在业务层合并结果。
        ...
        - 方案二：可以将经常使用到 group by,order by 字段存储到一个单一库表(可以是 REDIS、ES、MYSQL)中，业务代码中先到单一表中根据查询条件查询出相应数据，然后根据查询到的主键 ID，到分库分表中查询详情进行返回。2 次查询操作难点会带来接口耗时的增加，以及极端情况下的数据不一致问题。

#### 分库分表后，分页查询问题

- 问题：分页查询需要从多个分片表合并数据，再统一分页，逻辑复杂度增加。

- 解决方案

    - 1.中间件支持分页：如 ShardingSphere。

    - 2.各分片分页后合并：先按分片分页查询，业务层合并排序后分页。
...
- 例子：订单表_1 的主键从 1 开始，订单表_2 的主键也从 1 开始。
...
- 解决方案
    ...
    - 3.分布式ID：自己实现一套分布式ID生成算法或者使用开源的如twitter的雪花算法
        ...
        - 百度的分布式ID UidGenerator：基于 Snowflake（雪花算法） 的改进版。
        - 美团的分布式ID，Leaf：开源，号段模式和 Snowflake 双支持。
